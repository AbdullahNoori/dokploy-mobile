...
<Info>
  For free users, enable safe area classNames by installing `react-native-safe-area-context`, wrapping your root layout with `SafeAreaListener`, and calling `Uniwind.updateInsets(insets)` on change.
  Find more in [FAQ](/faq#how-do-i-enable-safe-area-classnames).
</Info>
